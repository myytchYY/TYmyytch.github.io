...
def binpack(articles,bin_cap):
   ...
   """
    ...
    while len(item_list)>=1:
        ...
        item = item_list.pop(); vol = item[1]; added = False
        for b in currBin:
            if vol+b[1] <= bin_cap:
                b[0].append(item[0]);b[1] +=vol;added = True
                break
        if added !=True: currBin.append([[item[0]],vol])
    for b in currBin:
...
def getDBDataList(commandString):
    #cnx = mySQL.connect(user=mysq l_user_name, passwd=mysql_password,
